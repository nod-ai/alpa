...
from chemprop.features import BatchComputeGraph, get_node_fdim, get_edge_fdim, graph2batch
...
class MPNEncoder(nn.Module):
    ...
    def forward(self,
        ...
        if atom_descriptors_batch is not None:
            if len(node_hiddens) != len(atom_descriptors_batch):
                raise ValueError(f'The number of atoms is different from the length of the extra atom features')

            node_hiddens = torch.cat([node_hiddens, atom_descriptors_batch], dim=1)     # num_atoms x (hidden + descriptor size)
            node_hiddens = self.atom_descriptors_layer(node_hiddens)                    # num_atoms x (hidden + descriptor size)
...
class MPN(nn.Module):
    ...
    def __init__(self,
        ...
        self.node_fdim = node_fdim or get_node_fdim(overwrite_default_atom=args.overwrite_default_atom_features,
                                                     is_reaction=(self.reaction or self.reaction_solvent))
        self.edge_fdim = edge_fdim or get_edge_fdim(overwrite_default_atom=args.overwrite_default_atom_features,
        ...
        if not self.reaction_solvent:
            if args.mpn_shared:
                self.encoder = nn.ModuleList([MPNEncoder(args, self.node_fdim, self.edge_fdim)] * args.number_of_molecules)
            else:
                self.encoder = nn.ModuleList([MPNEncoder(args, self.node_fdim, self.edge_fdim)
            ...
            self.node_fdim_solvent = get_node_fdim(overwrite_default_atom=args.overwrite_default_atom_features,
                                                   is_reaction=False)
            self.edge_fdim_solvent = get_edge_fdim(overwrite_default_atom=args.overwrite_default_atom_features,
                                                   overwrite_default_bond=args.overwrite_default_bond_features,
                                                   node_messages=args.node_messages,
                                                   is_reaction=False)
            self.encoder_solvent = MPNEncoder(args, self.node_fdim_solvent, self.edge_fdim_solvent,
                                               args.hidden_size_solvent, args.bias_solvent, args.depth_solvent)

    def forward(self, batch: List[List[HLOGraph]]) -> torch.FloatTensor:
        """
